...
def test_config_backwards_compatibility(tmp_path):
    ...
    test_ver = 'v1.0.0'
    old_fpath = Path(f'test/fixtures/test_config/config_full_output_Fy4_{test_ver}.yaml')    
    with pytest.raises(NotImplementedError):
        FFConfig.load_or_init(load_path=old_fpath, default_fpath=default_fpath,
                              init_debug=False, init_version=last_ver)
    
    test_ver = 'v1.0.2'
    old_fpath = Path(f'test/fixtures/test_config/config_{test_ver}_test.yaml')
    config = FFConfig.load_or_init(load_path=old_fpath, default_fpath=default_fpath,
                                   init_debug=False, init_version=last_ver)
    assert '-9999' in config.data_import.ias.missing_data_codes 
    assert config.data_import.csf.empty_co2_strg
    
    test_ver = 'v1.0.4'
    old_fpath = Path(f'test/fixtures/test_config/config_{test_ver}_default_ru.yaml')
    config = FFConfig.load_or_init(load_path=old_fpath, default_fpath=default_fpath,
                                   init_debug=False, init_version=last_ver)
